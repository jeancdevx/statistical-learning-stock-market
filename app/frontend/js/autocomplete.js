export class TickerAutocomplete {
  ...
  handleInput() {
    ...
    if (this.filteredTickers.length === 0) {
      this.list.innerHTML =
        '<div class="px-4 py-2 text-gray-500">No se encontraron resultados</div>'
      this.list.classList.remove('hidden')
      return
    }

    this.list.innerHTML = this.filteredTickers
      .map(
        (ticker, index) =>
          `<div class="autocomplete-item px-4 py-2 hover:bg-blue-50 cursor-pointer ${
    ...
    this.list.querySelectorAll('.autocomplete-item').forEach((item) => {
      item.addEventListener('click', () =>
        this.selectTicker(item.dataset.ticker)
      )
    })
  ...
  handleKeydown(e) {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      this.selectedIndex =
        (this.selectedIndex + 1) % this.filteredTickers.length
      this.updateSelection()
    } else if (e.key === 'ArrowUp') {
      e.preventDefault()
      this.selectedIndex =
        this.selectedIndex <= 0
          ? this.filteredTickers.length - 1
          : this.selectedIndex - 1
      this.updateSelection()
