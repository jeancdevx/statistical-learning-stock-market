import { backtest, fetchModels, fetchSystemHealth, fetchTickerDates, fetchTickerHistory, fetchTickers, predict } from '/js/api.js';
...
class StockPredictionApp {
    ...
    async loadTickers() {
        try {
            ...
            this.autocomplete = new TickerAutocomplete(tickerInput, tickerSuggestions, this.tickers);

            tickerInput.addEventListener('ticker-selected', () => this.handleTickerSelected());
        } catch (error) {
    ...
    async handleTickerSelected() {
        ...
        try {
            ...
            if (history) {
                ...
                document.getElementById('priceHistoryContainer')?.classList.remove('hidden');
            }
    ...
    changeTimeRange(range) {
        if (this.currentHistoryData) {
            ...
            document.querySelectorAll('#priceHistoryContainer button').forEach(btn => {
    ...
    setupEventListeners() {
        ...
        backtestCheckbox.addEventListener('change', () => this.updateDateValidation());
    ...
    updateDateValidation() {
        ...
        if (validation.mode === 'prediction' && !backtestCheckbox.checked) {
            ...
            dateWarning.textContent = 'No se puede hacer backtest del día siguiente (aún no hay datos reales)';
            dateWarning.classList.remove('hidden');
            predictBtn.disabled = true;
        } else {
            dateWarning.textContent = validation.message;
