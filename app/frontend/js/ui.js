export class UIManager {
    ...
    displayPrediction(direction, directionText, probability, confidence, dates) {
        ...
        this.elements.predictionCard.className = `text-center p-8 rounded-lg ${isUp ? 'bg-green-50 border-2 border-green-200' : 'bg-red-50 border-2 border-red-200'}`;
        this.elements.predictionCard.innerHTML = `
            <div class="text-6xl mb-4">${isUp ? '📈' : '📉'}</div>
            <div class="text-3xl font-bold ${isUp ? 'text-green-700' : 'text-red-700'} mb-2">
                ${directionText}
            </div>
            <div class="text-gray-600 text-sm">Predicción: ${direction === 1 ? '+1' : '0'}</div>
        `;

        const probabilityPercent = (probability * 100).toFixed(1);
        this.elements.probabilityBar.style.width = probabilityPercent + '%';
        this.elements.probabilityBar.className = `h-full rounded-full transition-all duration-500 ${isUp ? 'bg-green-500' : 'bg-red-500'}`;
        ...
        this.elements.confidenceLevel.className = `font-semibold ${confidenceColors[confidence] || 'text-gray-600'}`;
    ...
    displayBacktest(real, isCorrect) {
        ...
        this.elements.backtestCard.innerHTML = `
            <h4 class="font-semibold text-gray-800 mb-4 flex items-center">
                <svg class="w-5 h-5 mr-2 ${isCorrect ? 'text-green-600' : 'text-red-600'}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="${isCorrect ? 'M5 13l4 4L19 7' : 'M6 18L18 6M6 6l12 12'}"></path>
                </svg>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div class="bg-blue-50 p-4 rounded-lg text-center">
                    ...
                    <p class="text-2xl font-bold ${isRealUp ? 'text-green-600' : 'text-red-600'}">
                ...
                <div class="bg-green-50 p-4 rounded-lg text-center">
                    ...
                    <p class="text-sm ${changeClass}">
                        ${real.change_pct >= 0 ? '+' : ''}${real.change_pct.toFixed(2)}%
                    </p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg text-center">
                    <p class="text-xs text-gray-600 font-medium mb-2">Validación</p>
                    <p class="text-2xl font-bold ${isCorrect ? 'text-green-600' : 'text-red-600'}">
                        ${isCorrect ? '✓ CORRECTO' : '✗ INCORRECTO'}
                    </p>
                </div>
            </div>
            
            <div class="bg-gray-50 rounded-lg p-4 border-l-4 ${isCorrect ? 'border-green-500' : 'border-red-500'}">
                <p class="text-sm text-gray-700 mb-2">
                    <span class="font-semibold">Cierre del día anterior:</span> $${real.prev_close.toFixed(2)}
                </p>
                <p class="text-sm text-gray-700 mb-2">
                    <span class="font-semibold">Apertura del día siguiente:</span> $${real.open.toFixed(2)}
                </p>
        ...
        metricsElement.innerHTML = `
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="text-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <p class="text-lg font-bold ${priceChange >= 0 ? 'text-green-600' : 'text-red-600'}">
                        ${priceChange >= 0 ? '+' : ''}${priceChange.toFixed(2)}
                    </p>
                    <p class="text-xs ${priceChange >= 0 ? 'text-green-600' : 'text-red-600'}">
                        ${priceChange >= 0 ? '+' : ''}${priceChangePercent.toFixed(2)}%
                    </p>
                </div>
                <div class="text-center p-3 bg-gray-50 rounded-lg">
                    <p class="text-xs text-gray-600 mb-1">Rango del Día</p>
                    <p class="text-lg font-bold text-gray-800">$${dayRange.toFixed(2)}</p>
                    <p class="text-xs text-gray-600">${dayRangePercent.toFixed(2)}%</p>
                </div>
                <div class="text-center p-3 bg-gray-50 rounded-lg">
                    <p class="text-xs text-gray-600 mb-1">Volatilidad 5d</p>
                    <p class="text-lg font-bold text-gray-800">${(vol5 * 100).toFixed(2)}%</p>
                </div>
                <div class="text-center p-3 bg-gray-50 rounded-lg">
                    <p class="text-xs text-gray-600 mb-1">RSI (14)</p>
                    <p class="text-lg font-bold ${rsi > 70 ? 'text-red-600' : rsi < 30 ? 'text-green-600' : 'text-gray-800'}">
                        ${rsi.toFixed(1)}
                    </p>
